...
import type { AnalyticsSummary, Campaign } from "../../domain/models";
...
export function CampaignAnalyticsPage({ workspaceId }: { workspaceId: string }) {
  const { campaignId = "" } = useParams(); const [campaign, setCampaign] = useState<Campaign | null>(); const [summary, setSummary] = useState<AnalyticsSummary>(); const [error, setError] = useState("");
  useEffect(() => { const to = new Date(); const from = new Date(Date.UTC(to.getUTCFullYear(), to.getUTCMonth(), to.getUTCDate() - 29)); setError(""); void Promise.all([qrRepository.getCampaign(workspaceId, campaignId), qrRepository.getAnalytics(workspaceId, from.toISOString(), to.toISOString(), campaignId)]).then(([value, analytics]) => { setCampaign(value); setSummary(analytics); }).catch((cause) => setError(errorMessage(cause))); }, [campaignId, workspaceId]);
